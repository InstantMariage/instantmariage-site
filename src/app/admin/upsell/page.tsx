...
function computeScore(p: Omit<PrestataireStat, "score">): number {
  ...
  const anciennete =
    new Date().getTime() - new Date(p.created_at).getTime() > 7 * 86400_000 ? 10 : 0;
...
export default function AdminUpsellPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (!session) { setError("Non autorisé"); return; }
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-xl font-bold text-gray-900">Opportunités de conversion</h1>
        <p className="text-sm text-gray-500 mt-1">Prestataires actifs à convertir vers le plan supérieur</p>
      </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100">
                  <th className="text-left text-xs font-semibold text-gray-500 px-5 py-3.5">Prestataire</th>
                  <th className="text-left text-xs font-semibold text-gray-500 px-4 py-3.5">Plan actuel</th>
                  <th className="text-left text-xs font-semibold text-gray-500 px-4 py-3.5 min-w-[160px]">Score d&apos;activité</th>
                  <th className="text-center text-xs font-semibold text-gray-500 px-4 py-3.5">Documents</th>
                  <th className="text-center text-xs font-semibold text-gray-500 px-4 py-3.5">Messages</th>
                  <th className="text-center text-xs font-semibold text-gray-500 px-4 py-3.5">Complétude</th>
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-gray-50/50 transition-colors">
                      <td className="px-5 py-3.5">
                        <span className="font-medium text-gray-900">{p.nom_entreprise}</span>
                      </td>
                      ...
                      <td className="px-4 py-3.5 text-center">
                        <span className="font-semibold text-gray-700">{p.doc_count}</span>
                      </td>
                      <td className="px-4 py-3.5 text-center">
                        <span className="font-semibold text-gray-700">{p.msg_count}</span>
                      </td>
                      <td className="px-4 py-3.5 text-center">
