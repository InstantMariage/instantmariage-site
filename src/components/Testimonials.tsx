...
export default function Testimonials() {
  ...
  const { ref: badgeRef, visible: badgeVisible } = useScrollReveal(0.2);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div ref={gridRef} className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {cards.map((card, i) => (
            <div
              key={card.label}
              className="bg-white rounded-2xl p-8 shadow-card border border-rose-50 flex flex-col items-center text-center hover:shadow-lg hover:-translate-y-1 transition-all duration-200"
              style={{
                opacity: gridVisible ? 1 : 0,
                transform: gridVisible ? "translateY(0)" : "translateY(32px)",
                transition: `opacity 0.5s ease ${i * 0.12}s, transform 0.5s ease ${i * 0.12}s, box-shadow 0.2s ease, translate 0.2s ease`,
              }}
            >
        ...
        <div
          ref={badgeRef}
          className="mt-14 bg-white rounded-2xl shadow-card p-6 md:p-8"
          style={{
            opacity: badgeVisible ? 1 : 0,
            transform: badgeVisible ? "translateY(0)" : "translateY(24px)",
